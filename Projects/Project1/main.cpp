...
void plyCrap(int[][COLS],char&,int&,int&,int&,int);//Main game of craps
void simCrap(int[][COLS],int&,int&,int&,int&,int&,int);//Sim multiple games of craps
void payout(char,int&,int); //Doubles wager if win and keeps it if loss
void results(int[][COLS],int,int,int,int,int,int,int);//Display results of game
void resltFi(ofstream &,int[][COLS],int,int,int,int,int,int,int);//Output results to file
...
int main(int argc, char** argv) {
    ...
    const int SIZE=13;//Number of rows in the array
    int table[SIZE][COLS]={},//Initializing the win array
    ...
    while(menu!=menuSiz){
        ...
        switch(menu){
            //Plays the main game
            case 1:{
                plyCrap(table,winLose,wager,gameRol,mostRol,chips);
            ...
            case 3:{
                cout<<"How many games would you like to simulate, up to 1,000,000"<<endl;
                cin>>numSims;
                cout<<endl;
                numSims=numSims>maxSims?maxSims:numSims;//Limits games to initialized value
                int start=time(0);
                simCrap(table,gameRol,totlRol,mostRol,numWin,numLoss,numSims);
                totlGam=numWin+numLoss;
                int finish=time(0);
                totlTim=finish-start;
                results(table,SIZE,numWin,numLoss,totlRol,mostRol,totlTim,totlGam);
                break;
            }
            ...
            case 4:{
                ...
                resltFi(out,table,SIZE,numWin,numLoss,totlRol,mostRol,totlTim,totlGam);
                break;
            }
...
void resltFi(ofstream &out,int table[][COLS],int SIZE,int numWin,int numLoss,int totlRol,int mostRol,int totlTim,int totlGam){
    ...
    for(int sum=2;sum<SIZE;sum++){
        sWins+=table[sum][0];
        sLosses+=table[sum][1];
        out<<setw(4)<<sum<<setw(10)<<table[sum][0]<<setw(10)<<table[sum][1]<<endl;
    }
...
void results(int table[][COLS],int SIZE,int numWin,int numLoss,int totlRol,int mostRol,int totlTim,int totlGam){
    //Create 2D array table that outputs which rolls resulted in win/loss
...
void payout(char winLose,int& chips,int wager){
...
void plyCrap(int table[][COLS],char& winLose,int& wager,int& gameRol,int& mostRol,int chips){
    ...
    switch(frstSum){
        ...
        case 11:
            ...
            winLose='W';//Reference back to main
        ...
        case 12:
            ...
            winLose='L';//Reference back to main
